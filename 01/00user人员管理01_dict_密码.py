...
while True:
    ...
    if INPUT == "find":
        ...
        for i in users.items():
            if input_txt == i[1]['name']:
                print("|{0:<5}|{1:<8s}|{2:<3s}|{3:<11s}|{4:<15s}|".format("ID","姓名","年龄","联系方式","密码"))
                print("|{0:<5}|{1:<10s}|{2:<5d}|{3:<15s}|{4:<20s}|".format(i[0],i[1]['name'],i[1]['age'],i[1]['tel'],
                '*' * len(v['password'])))
            #print("此用户不存在！！！")
        #else:
    ...
    if INPUT == "list":
        ...
        for k,v in users.items():
            print("|{0:<5}|{1:<10s}|{2:<5d}|{3:<15s}|{4:<20s}|".format(k,v['name'],v['age'],v['tel'],'*' * len(v['password'])))
